...
if (!empty($_POST['ubah_password_validate'])) {
    $query = mysqli_query($conn, "SELECT * FROM user_project where username='$_SESSION[username_cafe]' && password='$passwordlama'");
    $hasil = mysqli_fetch_array($query);
    if ($hasil) {
        if ($passwordbaru == $repasswordbaru) {
            $query = mysqli_query($conn, "UPDATE user_project set password='$passwordbaru' where username='$_SESSION[username_cafe]'");
            if ($query) {
